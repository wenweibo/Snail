...
public class SellCarActivity extends BaseTitleActivity {
    ...
    protected void initData() {
        super.initData();
        gvTruckType.setAdapter(new TruckTypeAdapter(this, datas = getTypeData()));
    }
        ...
        switch (view.getId()) {
            case R.id.btn_next_step:
                ...
                if (datas != null && !datas.isEmpty()) {
                    ...
                    // 如果选择了车型，则进行下一步
                    if (selectedME != null) {
                        DictInfoEntity vehicleEntity = selectedME.getDictInfoEntity();
                        Bundle bundle = new Bundle();
                        bundle.putSerializable("vehicleEntity", vehicleEntity);
                        startActivity(bundle, SellBaseInputActivity.class);
                    } else {
                        // 如果没有选择车型，则弹出提示
    ...
    private List<MenuEntity> getTypeData() {
        ...
        for (int i = 0; i < truckImgArr.length; i++) {
            ...
            menuEntity.setTitle("");
            menuEntity.setId(i + "");
            menuEntity.setDictInfoEntity(DictInfo.vehicleTypes.get(i));
