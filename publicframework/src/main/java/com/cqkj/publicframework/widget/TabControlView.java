...
public class TabControlView extends RadioGroup {
        ...
        public void onCheckedChanged(RadioGroup group, int checkedId) {
            if (mListener != null) {
                ...
                for (int i = 0; i < mOptions.size(); i++) {
                    RadioButton radioButton = mOptions.get(i);
                    if (radioButton.getText().toString().equals(value)){
                        mPosition = i;
                    }
                }
                mListener.newSelection(mPosition);
            }
        }
    };
    ...
    private void initAttrs(Context context, AttributeSet attrs) throws Exception {
        ...
        TypedArray attributes = context.getTheme().obtainStyledAttributes(
                ...
                0, 0);
        try {
            mTextSize = attributes.getDimensionPixelSize(R.styleable.TabControlView_tcv_textSize, getDimensionPixelSize(R.dimen.tab_text_size));
            mSelectedColor = attributes.getColor(R.styleable.TabControlView_tcv_selectedColor, getColor(R.color.colorPrimary));
            ...
            mUnselectedTextColor = attributes.getColor(R.styleable.TabControlView_tcv_unselectedTextColor, getColor(R.color.colorPrimary));
            mStrokeWidth = attributes.getDimensionPixelSize(R.styleable.TabControlView_tcv_strokeWidth, getDimensionPixelSize(R.dimen.line));
            mOptionWidth = attributes.getDimensionPixelSize(R.styleable.TabControlView_tcv_optionWidth, getDimensionPixelSize(R.dimen.img_size_20));
     ...
     * 通过下标 设置选中的Tab
     */
    public TabControlView setSelection(int position) {
        if (position > mOptions.size() || position < 0){
            return null;
        }
    ...
    public interface OnTabSelectionChangedListener {
        ...
        void newSelection(int position);
    }
